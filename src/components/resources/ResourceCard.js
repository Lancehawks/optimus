...
export default function ResourceCard({ resource, onEdit, onDelete }) {
  const dropdownItems = [
    { label: "Edit", onClick: () => onEdit(resource) },
  ];
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-heading! font-medium truncate">{resource.title}</h3>
            {resource.notes && (
              <p className="text-body-sm text-muted! mt-1 line-clamp-2">{resource.notes}</p>
            )}
            <div className="flex items-center gap-2 mt-2">
              <Badge variant="neutral" size="sm">{resource.type.toUpperCase()}</Badge>
              <span className="text-caption text-muted!">{timeAgo(resource.created_at)}</span>
            </div>
          </div>
        </div>
        <Dropdown
